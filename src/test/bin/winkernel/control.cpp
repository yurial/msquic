    ...
    )
{
...
#ifdef QUIC_API_ENABLE_PREVIEW_FEATURES
    RegisterTestFunction(QuicTestConnReceivePauseResume);
    RegisterTestFunction(QuicTestConnMaxDataLoweredBlocksSend);
    RegisterTestFunction(QuicTestStreamRecvPauseBlocksSend);
    RegisterTestFunction(QuicTestRecvPauseDeferredCredit);
#endif
    ...
    char Buffer[256];
    (void)_snprintf_s(Buffer, sizeof(Buffer), _TRUNCATE,
